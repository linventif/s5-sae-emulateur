...
func initCPUState(state *CPUState, firstInstruction uint32, defaultMemoryValue uint32) {
	for i := 0; i < 32; i++ {
		state.x[i] = defaultMemoryValue
	}
	state.pc = firstInstruction
	logDebug("INIT", "CPU state initialized with default memory value %d\n", defaultMemoryValue)
}

// readRegister lit un registre
func readRegister(state *CPUState, reg int) uint32 {
	if reg == 0 {
		return 0 // Le registre x0 vaut toujours 0
	}
	return state.x[reg]
}

// writeRegister écrit dans un registre
func writeRegister(state *CPUState, reg int, value uint32) {
	if reg != 0 {
		state.x[reg] = value // Écrire dans x0 ne change pas l'état du processeur
	}
}
...
//    initCPUState(&cpu)
//
//    fmt.Println("Initial state of x0:", readRegister(&cpu, 0))
//    fmt.Println("Initial state of x1:", readRegister(&cpu, 1))
//
//    writeRegister(&cpu, 1, 42)
//    fmt.Println("State of x1 after writing 42:", readRegister(&cpu, 1))
//
//    writeRegister(&cpu, 0, 42)
//    fmt.Println("State of x0 after writing 42:", readRegister(&cpu, 0))
//}
...
func extractOptions() (string, uint32, uint32) {
	size := uint32(512 * 1024) // 512 Ko par défaut
	defaultMemoryValue := uint32(0)
	for i := 2; i < len(os.Args); i++ {
		if os.Args[i] == "-m" {
			i++
			size = 1024
			fmt.Sscanf(os.Args[i], "%d", &size)
			size *= 1024
		} else if os.Args[i] == "-d" {
			i++
			fmt.Sscanf(os.Args[i], "%d", &defaultMemoryValue)
		}
	}
	return os.Args[1], size, defaultMemoryValue
}
...
func main() {
	...
	var testOption = []string{"-m", "512", "-d", "0", "/home/linventif/sae-emulateur/riscv-samples/assembly/test1.bin"}
	os.Args = append(os.Args, testOption...)
	debugMode = true

	// default memory size & default memory value
	var memorySize uint32 = 512
	...
	for i, arg := range os.Args {
		...
		if arg == "-m" {
			if i+1 < len(os.Args) {
				fmt.Sscanf(os.Args[i+1], "%d", &memorySize)
			}
		}

		if arg == "-d" {
			if i+1 < len(os.Args) {
				fmt.Sscanf(os.Args[i+1], "%d", &registerDefault)
			}
		}
	}
	...
	// extract filename from last argument
	...
	for {
		...
		if offset == 0 {
			initCPUState(&cpu, instruction, registerDefault)
		}
